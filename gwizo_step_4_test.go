...
func TestStep_4(t *testing.T) {
  ...
  for i := 0; i < len(input); i++ {
    ...
    if octopus.Step_4() != stem[i] {
      t.Error(fmt.Sprintf("Test For %s ~~Failed~~ [%s != %s]", input[i], octopus.Step_4(), stem[i]))
    }else {
      t.Log(fmt.Sprintf("Test For %s **Passed** [%s == %s]", input[i], octopus.Step_4(), stem[i]))
    }
  }
}
